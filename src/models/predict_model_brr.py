...
class PredictModel:
    def __init__(self):
        ...
        self.brr = jb.load(f'../../models/brr.pkl')
    ...
    def predict_model(self):
        # Apply PCA on the data and Load the Model
        pca = PCA(n_components=9)
        pca_x_test = pca.fit_transform(self.test_x)
        flat_y_test = np.ravel(self.test_y)
        predict_y = self.brr.predict(pca_x_test)
        ...
        title = "BRR Residual Loss (Test data)"
        ...
        title = "BRR Prediction error distribution (Test data)"
