...
class Visualize:
    ...
    def plot_multi_kde(df, columns, super_title):
        ...
        fig, ax = plt.subplots(row, column, figsize=(19, 24))
        for feature in columns:
            x_label = str(feature).replace('_', ' ').capitalize().title()

            if i < row and j < column:
                sb.kdeplot(df[feature], ax=ax[i, j], color='#ab0516')
                ax[i, j].set_xlabel(x_label, fontsize=9, fontweight='bold')
                ax[i, j].set_ylabel('Density', fontsize=9, fontweight='bold')
    ...
    def plot_compare_kde(df1, df2, df3, columns, super_title):
        ...
        for feature in columns:
            ...
            if i < row and j < column:
                sb.kdeplot(df1[feature], ax=ax[i, j], color='#b34802')
                sb.kdeplot(df2[feature], ax=ax[i, j], color='#02b366')
                sb.kdeplot(df3[feature], ax=ax[i, j], color='#024ab5')
